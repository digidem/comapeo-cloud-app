...
- Route app-owned `localStorage` writes that create or update persisted state through the fenced helpers/Zustand adapter in `src/lib/comapeo-local-storage.ts`, so stale callbacks cannot recreate data during reset. Destructive removals used for cleanup are allowed; the durable reset coordination marker managed by `src/lib/storage-reset-coordinator.ts` and the one-time legacy credential cleanup marker `comapeo:credentialCleanupVersion` are intentional control-plane exceptions.
