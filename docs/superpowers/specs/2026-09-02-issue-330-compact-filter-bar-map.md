...
2. **Active-filter chips** (only while `obsFilters.isFiltering`):
   ...
   - Chip `✕` buttons keep `min-h-[44px]`/adequate touch targets per repo convention (this makes the bar ~52px tall — accepted) and an accessible name (`data.filters.removeFilter` message, e.g. "Remove filter: {label}"). `topLeftPositionClassName` stays `top-[4.25rem] left-3 right-20 items-start` (still clears the basemap switcher at `top-[4.25rem] right-3`; `items-start` preserved); no 44px relaxation for this pointer-first surface.
...
- A **new hook `src/hooks/useMediaQuery.ts`** (`export function useMediaQuery(query: string): boolean`, named export, function declaration, `window.matchMedia` + `change` listener; **lazily initialize from `matchMedia` on first render** — pattern-match `src/screens/Home/AreaMap.tsx:75-88` (the only matchMedia+lazy-init hook; `useIsDesktop`/`useResponsivePageSize` use `innerWidth`+`resize` and are NOT the pattern) — not a hard-coded `false`) gates the desktop/mobile drawer variant. Unit tests must `vi.spyOn(window, 'matchMedia')` because `tests/setup.ts:18` globally stubs it with `matches: false` and no-op listeners (precedent: `tests/unit/stores/theme-store.test.ts:44`); jsdom ignores media queries, so "mocking the viewport" is not a viable strategy.
...
- chip row has nowrap + `min-w-0 overflow-x-auto` and a scrollable aria-labelled container; the Filters button and result count carry `shrink-0`
...
- `variant="drawer"` renders `absolute inset-0`-style positioning within the parent, with no `fixed bottom-0 left-0 right-0` classes; the in-drawer overlay (absolute within the parent, below the category content) is present and there is NO viewport-fixed overlay. Note: the current overlay is positioned via inline `style` (`position: fixed; inset: 0; zIndex: 50`, CategoryFilterSheet.tsx:73-81), not classes — assert via `container.querySelector` + computed `style.position` (precedent: `CategoryFilterSheet.test.tsx:109`) or add a `data-*` hook; a class-list assertion alone cannot see it.
...
- **P1 Breakpoint mismatch** (`useIsDesktop` = lg/64rem vs map-view desktop = md/48rem): DECIDED — `useMediaQuery('(min-width: 48rem)')`; `useIsDesktop` must not gate this feature.
