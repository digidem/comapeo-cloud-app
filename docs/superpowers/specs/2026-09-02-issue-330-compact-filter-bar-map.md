...
- No changes to **any mobile behavior** — mobile map (bottomLeft Filters button + bottom-sheet `FilterSheet`, lines 381-431) and mobile grid (lines 543-591) render exactly as today.
...
### Desktop map view (≥ md breakpoint, 768px, matching the current `hidden md:block` wrapper — NOT the `lg` breakpoint used by `useIsDesktop`)
...
2. **Active-filter chips** (only while `obsFilters.isFiltering`):
   - One chip per active filter: search (`Search: "term" ✕`), start date (`From: YYYY-MM-DD ✕`), end date (`To: YYYY-MM-DD ✕`), and **one chip per selected category** (`Category: X ✕`). Dismissing a chip calls the matching existing setter immediately: search → `setSearch('')`; start date → `setStartDate(null)`; end date → `setEndDate(null)`; category chip X → `setCategories(filters.categories.filter(c => c !== X))` (equivalently `toggleCategory(X)`; specify one — recommend `toggleCategory(X)` since it is the single canonical toggle).
   - Chip `✕` buttons must have `min-h-[44px]`/adequate touch targets per repo convention and an accessible name (`data.filters.removeFilter` message, e.g. "Remove filter: {label}").
...
Clicking **Filters** opens `FilterSheet` as a **right-side drawer**: `Dialog.Content` positioned `fixed right-0 top-0 bottom-0`, width `w-[min(380px,90vw)]`, slide-in-from-right animation (add a `slideInRight` keyframe alongside the existing `slideUp`/`fadeIn` in `src/app/styles.css`; do not fork the component into a second Dialog). The drawer contains, in order: header (title + `Dialog.Close` ✕), category selection (the existing `CategoryFilterSheet` nested trigger row, as today), search input, From/To date inputs, result count — i.e. exactly the current sheet contents, repositioned. `showSort={false}` unchanged.
...
- The mobile bottom-sheet rendering must be byte-identical in behavior; the desktop variant is selected purely via CSS (`md:`-prefixed positioning classes) so the same JSX serves both, avoiding any JS breakpoint drift between `useIsDesktop` (lg) and CSS (md).
...
- search (1 if `filters.search !== ''`), startDate (1 if non-null), endDate (1 if non-null), categories (**1 if `filters.categories.length > 0`**, regardless of how many categories are selected).
...
- `src/hooks/useIsDesktop.ts` uses **lg (1024px)** — do not use it for this feature; the map-view desktop surface is `md`-scoped in CSS today and stays `md`-scoped.
...
- Rendered from DataScreen map view as `<FilterSheet variant="right" ... />` (desktop wrapper `hidden md:block` guarantees this instance is desktop-only; the mobile `bottomLeft` instance stays `variant`-less).
- Mobile grid-view call site (lines 572-590) untouched.

**Wiring — `src/screens/DataScreen.tsx` map-view branch only (lines 332-465):**

- Replace the `topLeft` `ObservationFilterBar` block (lines 336-354) with `CompactObservationFilterBar` inside the existing `hidden w-full md:block` wrapper; adjust `topLeftPositionClassName` as needed for the compact footprint (it may keep `right-20` to clear the map controls).
- Add a second `FilterSheet` instance with `variant="right"` inside the same desktop wrapper, sharing `filterDrawerOpen` state.
- `activeFilterCount` computed in DataScreen from `obsFilters.filters` per the Badge semantics formula.
...
| `src/screens/DataScreen.tsx` | map-view `topLeft`: swap in `CompactObservationFilterBar` + desktop `FilterSheet variant="right"`; compute `activeFilterCount` |
...
1. Desktop map view (viewport ≥ 768px): `topLeft` renders exactly one compact line — Filters button with badge (when filtering), inline chips, result count; no search/date/category inputs visible outside the drawer. The full `ObservationFilterBar` no longer renders in the map view.
...
8. Mobile (< 768px): map and grid filter UIs byte-identical behavior to current main; mobile badge still shows `filteredObs.length`.
...
`tests/unit/screens/DataScreen.test.tsx` (EXTEND, desktop map view cases; mock viewport ≥ 768px):

- map view shows `CompactObservationFilterBar` and NOT `ObservationFilterBar`'s search input
- Filters button opens drawer (`variant="right"` FilterSheet); Apply/ESC/outside-click closes
...
- mobile map (< 768px): Filters button + bottom sheet, badge = result count — unchanged
...
4. RED→GREEN: DataScreen desktop map-view wiring tests (compact bar renders, drawer opens/closes, chip dismiss hits setters, badge count, grid/mobile unchanged), then the DataScreen changes + `activeFilterCount`.
5. `npm run extract-messages`; fill `pt.json`/`es.json`; i18n test/check.
6. Full `npm test`, `npm run test:coverage` (≥80%), lints; update Storybook stories/screenshots only if CI visual baselines require it (prefer adding stories for the new component rather than changing existing baselines).
...
- **P1 Breakpoint mismatch** (`useIsDesktop` = lg/1024 vs map-view desktop = md/768): DECIDED — CSS-only variant selection; `useIsDesktop` must not gate this feature.
...
- **P2 Two FilterSheet instances sharing one `filterDrawerOpen` state**: specified — desktop instance inside the `hidden md:block` wrapper, mobile instance inside `md:hidden` wrapper; both bind the same state; only one is visible per breakpoint, so no focus/portal conflict.
- **P2 Drawer width/animation**: specified — `w-[min(380px,90vw)]`, `slideInRight` keyframe added to `styles.css`, z-index pattern unchanged (z-50 overlay / z-[51] content).
