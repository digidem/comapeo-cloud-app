...
4. Label application (`agent:ready-for-implementation` + `lane:implementation`) is a **separate, retryable step after the spec PR merges AND after every implementation-start-gate dependency has merged**. Its failure is reported as `blocked: labels`, not `blocked: spec`. Never claim implementation readiness from an unmerged local/spec file alone.
