...
When the user requests a specific reviewer or model, use it. Otherwise use an available strong independent reviewer when the environment supports one and the change risk justifies it. Before declaring reviewer paths exhausted, consult **live usage instrumentation** when available instead of learning every quota state through expensive failed reviews. Prefer provider-scoped `codexbar usage` checks; if CodexBar cannot report the provider, try `plan-check json`, then a current `~/.hermes/state/quota-heartbeat.json` only if its timestamp is fresh enough to represent this execution. Treat unsupported usage sources, parser failures, and stale quota state as unknown rather than exhausted. A direct current provider limit/429 with a reset time is authoritative; record it and avoid repeated probes until the reset unless newer live usage data shows availability. Read `references/claude-qwen-review.md` for the detailed availability rules that also apply to fallback selection.
