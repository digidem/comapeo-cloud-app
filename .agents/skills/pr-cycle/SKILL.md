...
7. Verify GitHub reports the PR merged and capture the resulting merge commit SHA. If the PR became merged without this execution issuing the merge command, report it as an external/concurrent merge and do not claim that this execution performed or was authorized to perform it. For squash or merge-commit methods used by this workflow, verify the resulting commit's **first parent** equals the exact gated base tip. Treat first-parent equality as post-merge **audit evidence, not a substitute for the pre-merge atomic base guard**; a mismatch is concurrent movement and must stop any subsequent ordered merge.
   - Treat a non-zero merge-command exit as ambiguous until live PR state is re-read. Some forge CLIs perform the server-side merge first and then fail during local `--delete-branch`/checkout cleanup, especially when the target branch is already checked out in another linked worktree. Never blindly retry the merge command; first verify PR state, merge commit SHA, and first-parent/base-tip evidence, then continue only with scoped cleanup if the merge already landed.
...
5. Remove the PR local branch after the worktree is gone and only after step 2 proved it safe. For a verified squash merge, `git branch -d` may correctly refuse because the feature commits are not ancestors of the squash commit; after merge verification and the exact tip-parity proof above, force-deleting **that exact PR branch only** with `git branch -D <branch>` is an allowed cleanup operation.
...
- `references/gpt-6-astra-review.md` — default GPT-6 Astra via Codex review path, exact-revision contract, resumable-session handling, and fallback rules.
