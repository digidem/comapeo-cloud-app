...
- For GPT-6 Astra via Codex (`gpt-6-astra`), a foreground `codex exec` timeout is not a verdict. If Codex emitted a resumable session id, resume that same session with GPT-6 Astra and request the terminal verdict; do not restart the full review or count partial reasoning as approval. See `gpt-6-astra-review.md`.
