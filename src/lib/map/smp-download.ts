...
async function mergeGlobalOverviewSmp(
  ...
  // Load directly from the already-buffered pass Blobs rather than creating
  // contiguous ArrayBuffer/Uint8Array copies first. This keeps the default
  // merge path within the same peak-heap envelope as the downloaded blobs.
...
async function loadEstimateBaseStyle(
  ...
  const downloader = new StyleDownloader(map.styleUrl);
...
export async function estimateAuthoredPayload(config: {
  ...
  const { map, authoredLayers, includeGlobalOverview = true, signal } = config;
  throwIfCallerAborted(signal);
  ...
  const baseStyle = await loadEstimateBaseStyle(map);
  throwIfCallerAborted(signal);
