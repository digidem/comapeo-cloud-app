...
describe('Case report disclosure persistence', () => {
  ...
  it('clears previously approved area material when a later disclosure omits it', async () => {
    await seed();
    const approvedArea = {
      ...
      provenance: {
        origin: 'independent-user-approved' as const,
        sourceId: 'community-boundary-1',
        approvedAt: '2026-09-05T12:00:00.000Z',
      },
    };
    ...
    const updated = await upsertCaseReportDisclosure({
      projectLocalId: 'project-1',
      caseLocalId: 'case-1',
      agency: 'FUNAI',
      disclosure: {
    ...
    expect(updated.approvedArea).toBeUndefined();
    expect(
      (await getCaseReportDisclosure('project-1', 'case-1', 'FUNAI'))
        .approvedArea,
    ).toBeUndefined();
